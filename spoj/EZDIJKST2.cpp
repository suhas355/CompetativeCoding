...
void findShortestPath(int **adj,int *dist,bool *visited,int v,int e){
	//1. get adj matrix
...
void flushIt(int **adj,int *dist,bool *visited,int v){
	for(int i=0;i<=v;i++){
		dist[i]=-1;
		visited[i]=false;
		for(int j=0;j<=v;j++)
...
int main(){
	...
	int **adj=getMatrix(10005);
	int *dist = (int *)malloc(sizeof(int)*(10005));
	bool *visited=(bool *)malloc(sizeof(bool)*(10005));
	...
	while(t--){
		//cout <<"-----------------------------" <<endl;
		cin >>v >>e;
		flushIt(adj,dist,visited,v);
		findShortestPath(adj,dist,visited,v,e);		
	}
